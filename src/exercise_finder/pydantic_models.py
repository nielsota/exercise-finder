...
class AgentMultipartQuestionOutput(BaseModel):
    ...
    This is what the LLM agent returns - the formatted text content plus difficulty.
    The agent's job is to parse question text into structured parts (title, stem, parts)
    and assess difficulty (1-5 scale).
    ...
    difficulty: int = Field(..., ge=1, le=5, description="Difficulty rating 1-5 (1=easy, 5=hard)")
...
class MultipartQuestionOutput(AgentMultipartQuestionOutput):
    ...
    Note: difficulty is inherited from AgentMultipartQuestionOutput (generated by agent).
